...
namespace pdaaal {
    ...
    public:
        ...
        CegarPdaFactory(std::function<uint32_t(const Label&)>&& label_abstraction_fn,
                        std::function<AbstractState(const State&)>&& state_abstraction_fn)
                : _temp_pda(std::move(label_abstraction_fn)), _state_abstraction(std::move(state_abstraction_fn)),
                  _rule_abstraction([this](const concrete_rule_t& r) {
                      abstract_rule_t res;
                      res._from = this->_state_abstraction.insert(r._from).second;
                      res._to = this->_state_abstraction.insert(r._to).second;
                      res._pre = this->_temp_pda.insert_label(r._pre).second;
                      res._op_label = (r._op == PUSH || r._op == SWAP) ? this->_temp_pda.insert_label(r._op_label).second : std::numeric_limits<uint32_t>::max();
                      res._op = r._op;
                      return res;
                  }) { };
        ...
        AbstractionSolverInstance<Label,W,C,A> compile(const NFA<Label>& initial_headers, const NFA<Label>& final_headers) {
            build_pda(_temp_pda);
            return AbstractionSolverInstance<Label,W,C,A>{pda_t{std::move(_temp_pda)}, initial_headers, _initial_states, final_headers, _accepting_states};
        }
        ...
        void reconstruct_trace(const AbstractionSolverInstance<Label,W,C,A>& instance, const NFA<Label>& initial_headers, const NFA<Label>& final_headers) {
            ...
            if constexpr (is_weighted<W>) {
                ...
                wildcard_header header_handler(initial_headers, final_headers, instance.pda(),
    ...
    protected:
        ...
        virtual bool accepting(State) = 0;
        virtual std::vector<std::pair<bool,concrete_rule_t>> rules(State) = 0;
            ...
            std::variant<header_t, std::pair<std::vector<Label>,std::vector<Label>>> finalize_header(const header_t& input_header) const {
                ...
                while (header.top_is_concrete()) {
                    if (!check_nfa_path(_final_nfa, _final_path, header.concrete_part.back(), i)) {
                        ...
                        return {std::in_place_index<1>, // Create refinement info
        ...
        private:
            std::variant<header_t, std::pair<std::vector<Label>,std::vector<Label>>> concreterize_wildcards(header_t&& header) const {
                ...
                for(; header.count_wildcards > 0; --header.count_wildcards) {
                    ...
                    if (label_it == label_end) {
                        ...
                        return {std::in_place_index<1>, initial_ok_labels, final_ok_labels};
                    } else {
            ...
            bool intersect_edge_labels(const NFA<Label>& nfa, const std::vector<const nfa_state_t*>& nfa_path, const std::vector<Label>& labels, size_t i) const {
                return i == 0
        ...
        using header_t = typename wildcard_header::header_t;

        virtual std::pair<conf_it,conf_it> first_confs(const wildcard_header&, std::vector<uint32_t>, const abstract_rule_t&) = 0;
        virtual std::pair<conf_it,conf_it> next_confs(const wildcard_header&, const configuration_t&, const abstract_rule_t&) = 0;
